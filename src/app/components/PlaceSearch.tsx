...
function sortResults(results: SearchResult[]) {
  return [...results].sort((a, b) => { return getPriority(b) - getPriority(a) })
}
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      ...
      try {
        ...
        if (!apiKey)
          throw new Error('NEXT_PUBLIC_MAPTILER_API_KEY is not defined')
  ...
  return (
    <div className="relative w-full max-w-xl">
      ...
      <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-xl">
        <svg className="h-5 w-5 shrink-0 text-slate-400" viewBox="0 0 24 24"
        ...
        <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search a city, country or place..."
